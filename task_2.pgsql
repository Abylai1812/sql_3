...
FROM Customers
WHERE country IS NOT NULL OR country IN (SELECT DISTINCT country FROM Suppliers)
ORDER BY country ASC;

-- 14. Выбрать такие страны в которых "зарегистированы" одновременно и заказчики и поставщики и работники.
SELECT country
FROM Customers
WHERE customer_id IN (SELECT customer_id FROM Orders) AND country IN (SELECT country FROM Suppliers) AND country IN (SELECT country FROM Employees);
...
SELECT country
FROM Customers
WHERE customer_id IN (SELECT customer_id FROM Orders) AND country IN (SELECT country FROM Suppliers) AND country NOT IN (SELECT country FROM Employees);
